#!/usr/bin/env python3
"""Build feature-fabric snapshot bundles from raw JSONL."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build fabric.v2 feature snapshots.")
    ...
    parser.add_argument(
        ...
        help="Snapshot mode label (for manifest metadata).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Output root for feature manifests.",
    )
    ...
    validated = validate_raw_dataset_jsonl_against_contract(
        raw_jsonl=raw,
        as_of_time=as_of,
        source="script_build_feature_fabric_snapshot",
    )
    ...
    if args.contract_manifest_root is not None:
        ...
        bundle = bundle.model_copy(update={"manifest_id": contract_manifest["manifest_id"]})
